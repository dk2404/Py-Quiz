from flask
import render_template, flash, redirect, url_for, request
from app
import app, db
from app.forms
import LoginForm, RegistrationForm
from flask_login
import current_user, login_user, logout_user, login_required
from app.models
import User, Question, Result, feedback, MCQ, answers
from werkzeug.urls
import url_parse
from sqlalchemy
import func
...
@app.route('/')
@app.route('/index', methods = ['GET', 'POST'])# @login_required
def index():
...
questions = Question.query.filter_by(text_question = True)# variables
for the total mark
for the long question gotten, total mark weighting#
for the long questions, percentage
for the mark and the response from admin
quizcompleteflag = True
markflag = False
mark = 0
question_mark = 0
responses = ""#
calculate the above variables below
for each long answer question
for question in questions: #if no long answer entry can be found
for the current user then skip the loop
if not bool(current_user.ques.filter_by(question_id = question.id).first()):
  quizcompleteflag = False
break#
  if no long answer entry can be found
for the current user then skip the loop
...
if there are entries in the long answer
...
numUser = 0
avglong = 0
numUseranswer = 0# calculate the user 's avg marks for short answer question
for user in users:
...
if request.method == "POST":
  ...
  #check
if the answer submitted are correct
for the short answer questions and generate the result accordingly
for question in questions:
...
# check
if quiz has been done by a user previously
if not add a quiz table
for the result of the user
...
# quiz route
for quiz.html the page where users get to play the quiz
